...
public class ConditionalCFGParser implements Scorer, KBestViterbiParser, Serializable {
  ...
  protected final ConditionalLexicon lex;
        ...
        for (int s = 0; s < numStates; s++) {
          ...
          for (BinaryRule br  : rules) {
            ...
            for (int split = min; split <= max; split++) {
              ...
              float totL = pS + rS + oS + lex.score(br,start,end,split);
        ...
        for (int s = 0; s < numStates; s++) {
          ...
          for (BinaryRule br : rules) {
            ...
            for (int split = min; split <= max; split++) {
              ...
              float totR = pS + lS + oS + lex.score(br,start,end,split);
              if (totR > oScore[split][end][br.rightChild]) {
                oScore[split][end][br.rightChild] = totR;
              }
            }
...
*/
  ...
  private Tree extractBestParse(int goal, int start, int end) {
    ...
    for (int split = start + 1; split < end; split++) {
      for (Iterator<BinaryRule> binaryI = bg.ruleIteratorByParent(goal); binaryI.hasNext(); ) {
        ...
        double score = br.score + iScore[start][split][br.leftChild] + iScore[split][end][br.rightChild] + lex.score(br, start, end, split);
        boolean matches;
        if (op.testOptions.lengthNormalization) {
  ...
  public ConditionalCFGParser(BinaryGrammar bg, UnaryGrammar ug, ConditionalLexicon lex, Options op, Index<String> stateIndex, Index<String> wordIndex, Index<String> tagIndex) {
